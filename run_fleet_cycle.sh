...
USABLE="$(python3 account_usage.py --accounts "$ALL_ACCOUNTS" --max-session-pct "$MAX_SESSION_PCT" | tail -1)"

CYCLE_ARGS=("$@")

if [ -z "$USABLE" ]; then
    echo "no accounts under ${MAX_SESSION_PCT}% session usage -- running cycle with --skip-dispatch"
    exec python3 fleet_cycle.py --accounts "$ALL_ACCOUNTS" --skip-dispatch "${CYCLE_ARGS[@]}"
else
    echo "usable accounts: $USABLE"
    exec python3 fleet_cycle.py --accounts "$USABLE" "${CYCLE_ARGS[@]}"
fi
